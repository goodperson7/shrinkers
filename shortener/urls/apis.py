...
from shortener.models import ShorteneUrls, Users, Statistic
from shortener.urls import serializers
from shortener.urls.serializers import UrlCreateSerializer, UserSerializer, UrlListSerializer,BrowerStatSerializer
...
class UrlListView(viewsets.ModelViewSet):
    ...
    """        
    queryset = ShorteneUrls.objects.all().order_by("-created_at")
    serializer_class = UrlListSerializer
    permission_classes = [permissions.IsAuthenticated]

    def create(self, request):
        # POST METHOD
        serializers = UrlCreateSerializer(data=request.data)
        if serializers.is_valid():
            rtn = serializers.create(request, serializers.data)
